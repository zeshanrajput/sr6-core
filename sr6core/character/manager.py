...
Handles discovering, loading, auditing, and exporting character portfolios.
"""
...
class CharacterManager:
    ...
    def discover_characters(self) -> Dict[str, Dict[str, Any]]:
        ...
        for char_id, cfg in self._character_configs.items():
            ...
            candidates.append(os.path.join(core_root, "characters", char_id))
            ...
            candidates.append(os.path.join(core_root, repo_name))
            ...
            for alias in legacy_aliases.get(char_id, []):
                candidates.append(os.path.join(core_root, "characters", alias))
                if self.github_root:
    ...
    def export_character(self, char_id: str, fmt: str = "quick_sheet", output_path: Optional[str] = None) -> Any:
        data = self.get_character_data(char_id)
        if not data:
        ...
        fmt = fmt.lower()
        if fmt in ["quick_sheet", "sheet", "text", "txt", "text_modular", "vtt", "base"]:
            from sr6core.publishing.quick_sheet import export_quick_sheet
            return export_quick_sheet(data, char_repo_path=repo_path)
...
def get_character_manager(config_path: Optional[str] = None) -> CharacterManager:
    """Returns a singleton/configured CharacterManager instance."""
    return CharacterManager(config_path=config_path)
