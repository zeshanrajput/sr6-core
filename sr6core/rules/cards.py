...
from sr6core.rules.db import DEFAULT_DB_PATH, RulesDB
...
def format_card(card: Dict[str, Any], fmt: str = "markdown") -> str:
    """Formats a card dictionary as Markdown or clean plaintext."""
    if fmt == "markdown":
        return card.get("markdown", "")

    name = card.get("name", "Item")
    cat = (card.get("category") or "Item").replace("_", " ").title()
    lines = [f"[{cat.upper()}] {name}"]
    stats = card.get("stats", {})
    if stats:
        stat_strs = [f"{k.upper()}: {v}" for k, v in stats.items() if v not in [None, "", "-"]]
        lines.append("  " + " | ".join(stat_strs))
    mods = card.get("modifications", [])
    if mods:
        lines.append(f"  MODS: {', '.join(str(m) for m in mods)}")
